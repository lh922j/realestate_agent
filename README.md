...
- **ML 분석 내장** — LightGBM 가격 예측(R² 0.882), Isolation Forest 이상거래 탐지를 자연어 한 마디로 실행
- **디버그 모드** — 사이드바 토글로 `[agent] → [tools] → [agent]` 노드 단위 실행 흐름을 UI에서 시각화
...
사이드바의 **🐛 디버그 모드** 토글을 켜면 에이전트 실행 흐름을 단계별로 확인할 수 있습니다.

| 단계 | 내용 |
|------|------|
| `[agent]` | 호출한 도구명과 파라미터 |
| `[tools]` | DB에서 가져온 실제 데이터 |
| `[agent]` | LLM이 생성한 최종 답변 |

`graph.stream(stream_mode="updates")`로 구현. 도구 선택 오류나 빈 결과 등 오동작 원인을 즉시 파악할 수 있습니다.
